                  ...
                  Boundary& outer_bnd, vector<Side>& sides, vector<SidesToBoundaryMap>& inner_dom_desc)
{
     ...
     * 
     * n = 0: Two non touching inner domains, smoothed half circles. Outer domain is a circle.
     * n = 1: Two non touching inner domains, half circles. Outer domain is a circle.
     * n = 2: Two touching inner domains, half circles. Outer domain is a circle.
     * n = 3: One inner domain, a circle. Outer domain is a circle.
     * n = 4: A spiral of cells. 
     * n = 5: One cluster of ten cells having shape similar to real ones.
     * n = 6: A rectangular cluster of cells. Number of cells, size, shape etc
     *        are specified via command line
     * n = 7: The microcard logo
     */    
    ...
    if(n==0)
    {
        ...
        outer_bnd = Boundary::get_circle({0.,0.},4.,dG,1.,0);
        Point p{0.2,0.};
        Real r = 0.2;
        Side s0 = -p+Arc({-r,0.},2.,0.5*M_PI,1.5*M_PI,dG,1.,kappa_l);
        Side s1 = -p+Arc({-r,-2.+r},r,-0.5*M_PI,0.,dG,1.,kappa_l);
        Side s2 = -p+Segment({0.,-2.+r},{0.,2.-r},dG,1.,kappa_l);
        Side s3 = -p+Arc({-r,2.-r},r,0.,0.5*M_PI,dG,1.,kappa_l);
        
        Side s4 = p+Arc({r,0.},2.,-0.5*M_PI,0.5*M_PI,dG,1.,kappa_l);
        Side s5 = p+Arc({r,2.-r},r,0.5*M_PI,M_PI,dG,1.,kappa_l);
        Side s6 = p+Segment({0.,2.-r},{0.,-2.+r},dG,1.,kappa_l);       
        Side s7 = p+Arc({r,-2.+r},r,M_PI,1.5*M_PI,dG,1.,kappa_l);
        ...
        inner_dom_2[2]={6,1};
        inner_dom_2[3]={7,1};
        vector<Side> sides_={s0,s1,s2,s3,s4,s5,s6,s7};
        vector<SidesToBoundaryMap> inner_dom_desc_ = {inner_dom_1,inner_dom_2};
        sides = sides_;     
        inner_dom_desc = inner_dom_desc_;
    ...
    if(n==1)
    {
        ...
        outer_bnd = Boundary::get_circle({0.,0.},4.,dG,1.,0);
        Point p{0.2,0.};
        Side s0 = -p+Arc({0.,0.},2.,0.5*M_PI,1.5*M_PI,dG,1.,kappa_l);
        Side s1 = -p+Segment({0.,-2.},{0.,2.},dG,1.,kappa_l);
        Side s2 = p+Arc({0.,0.},2.,-0.5*M_PI,0.5*M_PI,dG,1.,kappa_l);
        Side s3 = p+Segment({0.,2.},{0.,-2.},dG,1.,kappa_l);       
        ...
        inner_dom_2[0]={2,1};
        inner_dom_2[1]={3,1};
        vector<Side> sides_={s0,s1,s2,s3};
        vector<SidesToBoundaryMap> inner_dom_desc_ = {inner_dom_1,inner_dom_2};
        sides = sides_;     
        inner_dom_desc = inner_dom_desc_;
    ...
    if(n==2)
    {
        ...
        outer_bnd = Boundary::get_circle({0.,0.},4.,dG,1.,0.);
        Side s0 = Arc({0.,0.},2.,0.5*M_PI,1.5*M_PI,dG,1.,kappa_l);       
        Side s1 = Segment({0.,-2.},{0.,2.},dG,1.,kappa_l);
        Side s2 = Arc({0.,0.},2.,-0.5*M_PI,0.5*M_PI,dG,1.,kappa_l);
        SidesToBoundaryMap inner_dom_1(2);
        inner_dom_1[0]={0,1};
        inner_dom_1[1]={1,1};
        SidesToBoundaryMap inner_dom_2(2);
    ...
    if(n==3)
    {
        ...
        outer_bnd = Boundary::get_circle({0.,0.},4.,dG,1.,0.);
        Side s0 = Arc({0.,0.},2.,0.,2*M_PI,dG,1.,kappa_l);
        SidesToBoundaryMap inner_dom_1(1);
        inner_dom_1[0]={0,1};
        vector<Side> sides_={s0};
        vector<SidesToBoundaryMap> inner_dom_desc_ = {inner_dom_1};
        sides = sides_;     
        inner_dom_desc = inner_dom_desc_;
        return;
    }
...
//        Real u = 0.125; //cm.
        Real u = 5e-3; //cm.
        outer_bnd = Boundary::get_square({4.*u,4.*u},8.*u,dG,1.,0);
        
        // cells have side of length 2u=1e-2cm
        Segment h1 = Segment({0,0},{2.*u,0.},dG,1.,kappa_l);
        Segment h2 = Segment({0,0},{-2.*u,0.},dG,1.,kappa_l);
        Segment v1 = Segment({0,0},{0.,2.*u},dG,1.,kappa_l);
        Segment v2 = Segment({0,0},{0.,-2.*u},dG,1.,kappa_l);
        Segment d1 = Segment({0,0},{u,u},dG,1.,kappa_l);
        Segment d2 = Segment({0,0},{-u,-u},dG,1.,kappa_l);
        ...
        vector<Side> sides_={s0,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27};
        vector<SidesToBoundaryMap> inner_dom_desc_ = {inner_dom_1,inner_dom_2,inner_dom_3,inner_dom_4,inner_dom_5,inner_dom_6,inner_dom_7,inner_dom_8,inner_dom_9};
        sides = sides_;     
        inner_dom_desc = inner_dom_desc_;
        return;        
    }
...
//        Boundary outer_bnd = Boundary::get_ellipse({13*u,1*u},32.*u,16.*u,dG);
        outer_bnd = Boundary::get_rectangle({-7*u,-8.6*u},{33*u,10.6*u},dG,1.,0.);
        
        auto seg = [=](Real x1, Real y1, Real x2, Real y2){return Segment({x1*u,y1*u},{x2*u,y2*u},dG,1.,kappa_l);};
        sides.resize(0);
        ...
        sides.push_back(-Arc(c1*u,r*u,alpha,beta,dG,1.,kappa_l));
        ...
        sides.push_back(-Arc(c2*u,r*u,alpha,beta,dG,1.,kappa_l));
        ...
        sides.push_back(-Arc(c3*u,r*u,gamma,delta,dG,1.,kappa_l));
